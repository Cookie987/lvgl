...
static const lv_buttonmatrix_ctrl_t ime_kb_ctrl_chn_map[] = {
    LV_KEYBOARD_CTRL_BUTTON_FLAGS | 5, LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4),
    LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_KB_BTN(4), LV_BUTTONMATRIX_CTRL_CHECKED | 7,
    LV_KEYBOARD_CTRL_BUTTON_FLAGS | 6, LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3),
    LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_KB_BTN(3), LV_BUTTONMATRIX_CTRL_CHECKED | 7,
    LV_BUTTONMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_BUTTONMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1),
    LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_BUTTONMATRIX_CTRL_CHECKED | LV_KB_BTN(1),
    LV_BUTTONMATRIX_CTRL_CHECKED | LV_KB_BTN(1), LV_BUTTONMATRIX_CTRL_CHECKED | LV_KB_BTN(1),
    LV_KEYBOARD_CTRL_BUTTON_FLAGS | 2, LV_KEYBOARD_CTRL_BUTTON_FLAGS | 2, LV_BUTTONMATRIX_CTRL_CHECKED | 2, 6,
...
static const lv_buttonmatrix_ctrl_t default_kb_ctrl_spec_map[] = {
    ...
    LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_BUTTONMATRIX_CTRL_CHECKED | 2,
    LV_KEYBOARD_CTRL_BUTTON_FLAGS | 2, LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1), LV_KB_BTN(1),
    ...
    LV_KEYBOARD_CTRL_BUTTON_FLAGS | 2, LV_BUTTONMATRIX_CTRL_CHECKED | 2, 6, LV_BUTTONMATRIX_CTRL_CHECKED | 2,
...
void lv_keyboard_set_candidate_list(lv_obj_t * obj, lv_obj_t * list)
{
    ...
    if(keyboard->candidate_btnm && lv_obj_is_valid(keyboard->candidate_btnm)) {
        lv_obj_delete(keyboard->candidate_btnm);
    }
...
{
    lv_keyboard_t * keyboard = (lv_keyboard_t *)obj;
    const lv_buttonmatrix_ctrl_t * src = kb_ctrl[keyboard->mode];
    if(src == NULL) {
        return;
    }

    if(keyboard->popovers) {
        lv_buttonmatrix_set_ctrl_map(obj, src);
        return;
    }

    lv_buttonmatrix_t * btnm = (lv_buttonmatrix_t *)obj;
    lv_buttonmatrix_ctrl_t * ctrl_map = lv_malloc(sizeof(lv_buttonmatrix_ctrl_t) * btnm->btn_cnt);
    if(ctrl_map == NULL) {
        return;
    }

    lv_memcpy(ctrl_map, src, sizeof(lv_buttonmatrix_ctrl_t) * btnm->btn_cnt);
    for(uint16_t i = 0; i < btnm->btn_cnt; i++) {
        ctrl_map[i] &= (lv_buttonmatrix_ctrl_t)(~LV_BUTTONMATRIX_CTRL_POPOVER);
    }
...
static void lv_keyboard_ime_release_map(lv_keyboard_t * keyboard)
{
    if(keyboard->candidate_map) {
        lv_free((void *)keyboard->candidate_map);
        keyboard->candidate_map = NULL;
    }
}
...
static void lv_keyboard_ime_release_candidates(lv_keyboard_t * keyboard)
{
    ...
    if(keyboard->candidates_list) {
        for(int i = 0; i < keyboard->total_candidates; i++) {
            lv_free((void *)keyboard->candidates_list[i]);
        }
        lv_free((void *)keyboard->candidates_list);
        keyboard->candidates_list = NULL;
    }
...
static bool lv_keyboard_ime_ensure_candidate_btnm(lv_keyboard_t * keyboard)
{
    ...
    if(keyboard->candidate_btnm == NULL) {
        keyboard->candidate_btnm = lv_buttonmatrix_create(keyboard->candidate_list);
        if(keyboard->candidate_btnm == NULL) {
            return false;
        }
...
static void lv_keyboard_update_candidate_page(lv_keyboard_t * keyboard)
{
    ...
    const char ** map = lv_malloc(sizeof(char *) * (item_cnt + 1));
    lv_buttonmatrix_ctrl_t * ctrl_map = lv_malloc(sizeof(lv_buttonmatrix_ctrl_t) * item_cnt);
    ...
    if(has_prev) {
        ...
        ctrl_map[ctrl_idx++] = LV_BUTTONMATRIX_CTRL_CLICK_TRIG | 1;
    }
    ...
    for(int i = 0; i < visible; i++) {
        ...
        ctrl_map[ctrl_idx++] = LV_BUTTONMATRIX_CTRL_CLICK_TRIG | width;
    }
    ...
    lv_buttonmatrix_set_map(keyboard->candidate_btnm, map);
    lv_buttonmatrix_set_ctrl_map(keyboard->candidate_btnm, ctrl_map);
...
static void lv_keyboard_candidate_event_cb(lv_event_t * e)
{
    ...
    uint16_t btn_id = lv_buttonmatrix_get_selected_button(btnm);
    if(btn_id == LV_BUTTONMATRIX_BUTTON_NONE) {
        return;
    }
                                                 ...
                                                 const char * suffix, bool append_suffix)
{
    ...
    while(*p != '\0' && *out_cnt < out_max) {
        ...
        char * out = lv_malloc(total_len + 1);
        if(out == NULL) {
            p += char_len;
            continue;
        }
...
static bool lv_keyboard_trie_init(lv_keyboard_trie_t * trie, int32_t node_cap, int32_t link_cap)
{
    ...
    trie->nodes = lv_malloc(sizeof(lv_keyboard_trie_node_t) * node_cap);
    trie->links = lv_malloc(sizeof(lv_keyboard_trie_link_t) * link_cap);
...
static int lv_keyboard_collect_phrase_candidates(const char * pinyin, char *** out_list)
{
    ...
    char ** list = lv_malloc(LV_KEYBOARD_MAX_CANDIDATES * sizeof(char *));
    if(list == NULL) {
        return 0;
    }
    ...
    if(mix_char_candidates) {
        char ** extra = lv_malloc(LV_KEYBOARD_MAX_CANDIDATES * sizeof(char *));
        if(extra) {
...
static int lv_keyboard_collect_char_candidates(const char * pinyin, char ** out_list, int out_max)
{
    ...
    if(count < out_max) {
        char ** partial = lv_malloc(sizeof(char *) * out_max);
        if(partial) {
            int partial_cnt = 0;
